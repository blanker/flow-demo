...
fn main() -> Result<(), ffmpeg::Error> {
    ffmpeg::init().unwrap();
    let file = "bandicam-2025-03-25 11-53-20-251.mp4";
    ...
    let extension: Option<&str> = None;
    // let extension: Option<_> = Some("mp4");
    let filename_new = "60 — 3D Text with R3F-mask.mp4";
    let filename_png = match extension {
        None => format!("{filename_new}.png"),
        Some(ext) => {
            let f = &filename_new[..filename_new.len()-ext.len()-1];
            format!("{f}.png")
        },
    };
    println!("{}",filename_png);

    let meta =
        media_util::get_media_meta(file,
            ".".to_string(),"test.png".to_string(),
        );
